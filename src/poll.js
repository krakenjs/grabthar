...
|}) => {
  ...
  if (!nodeModulesDir) {
    throw new Error(
      `Can not find node modules for ${moduleDetails.modulePath}`
    );
  }
...
|}) => {
  const fullPath = path
    ? join(moduleDetails.modulePath, path)
    : moduleDetails.modulePath;
...
export function npmPoll({
  ...
  cdnRegistry,
  childModules,
}: NPMPollOptions): NpmWatcher<Object> {
  const pollers = {};
  ...
  async function withPoller<T>(
    handler: <T>(ModuleDetails) => Promise<T> | T,
    tag: ?string
  ): Promise<T> {
    if (tag && !pollers[tag]) {
  ...
  async function pollerImport<T: Object>(path, tag?: ?string): Promise<T> {
    return await withPoller(
      (moduleDetails) => importParent({ moduleDetails, path }),
      tag
    );
  }

  async function pollerImportDependency<T: Object>(
    dependencyName,
    path,
    tag?: ?string
  ): Promise<T> {
    return await withPoller(
      (moduleDetails) =>
        importDependency({ dependencyName, path, moduleDetails }),
      tag
    );
  }
  ...
  async function pollerRead(path?: string, tag?: ?string): Promise<string> {
    return await withPoller(
      (moduleDetails) => getFile({ moduleDetails, path }),
      tag
    );
  }
